...
from nets.ACGan_model import Generator as infoganG
from nets.ACGan_model import Discriminator as infoganD
    ...
    for epoch in range(resume_epoch,args.num_epochs+1):
        ...
        for batch_idx, sample  in enumerate(data_loader):
           ...
           # g_loss = netD.adv_loss(y_fake_g, real_label) + args.l1_p * g_reg_loss
            ...
            info_loss =  ( netD.class_loss(label_fake, x_fake_labels) +  netD.style_loss(style_fake, z_style) )/2
        ...
        with open(os.path.join(args.workspace_dir, 'training_loss.csv'),'w',newline='') as csvfile:
            ...
            for i in range(len(G_L_list)):
                c.writerow([i,G_L_list[i],D_L_list[i],Info_list[i]])
        if (epoch) % 50 == 0:
