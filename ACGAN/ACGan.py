...
import argparse
from torch.utils.data import WeightedRandomSampler as WeightedRandomSampler
from cat_dataset import CatDataset
from nets.ACGan_model import Generator as G
from nets.ACGan_model import Discriminator as D
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_D', type= int , default=0)
    parser.add_argument('--model_sample', type=int, default=50)
    parser.add_argument('--img_sample', type=int, default=5)


    args = parser.parse_args()

    dataset = CatDataset(args.dataset,args.data_dir)
    class_sample_count = np.unique(dataset.class_labels(), return_counts=True)[1]
    #print(class_sample_count)
    weight = 1. / class_sample_count
    samples_weight = weight[dataset.class_labels()]
    ...
    netG = G(classes , args.channels, args.img_size, args.latent_dim, args.style_dim)
    netG.apply(_weights_init)
    netG.cuda()

    netD = D(classes, args.channels, args.img_size, args.latent_dim, args.style_dim,batch_D=args.batch_D)
    ...
    D_L_list = []
    Info_list = []
    Epoch_l = range(args.num_epochs+1)
    for epoch in range(resume_epoch,args.num_epochs+1):
        ...
        for batch_idx, sample  in enumerate(data_loader):
            ...
            real_label = torch.ones([data.shape[0],]).cuda()
            fake_label = torch.zeros([data.shape[0],]).cuda()
            ...
            # Train D
            netD.zero_grad()
            y_real, label_true, _,_ = netD(data)
            d_real_loss = netD.adv_loss(y_real, real_label)
            d_real_class_loss = netD.class_loss(label_true,target)

            y_fake_d, label_fake, _ ,_= netD(x_fake.detach())
            d_fake_loss = netD.adv_loss(y_fake_d, fake_label)
            d_fake_class_loss = netD.class_loss(label_fake, x_fake_labels )
            #d_vector_loss = netD.reg_loss(y_fake_vec,y_real_vec)
            d_loss = (d_real_loss + d_fake_loss + args.l1_p*(d_real_class_loss+d_fake_class_loss)) / (2+2*args.l1_p)
            d_loss.backward()
            optim_D.step()
           ...
           # g_loss = netD.adv_loss(y_fake_g, real_label) + args.l1_p * g_reg_loss
            g_fake_loss  = netD.adv_loss(y_fake_g, real_label)
            g_fake_class_loss = netD.class_loss(label_fake, x_fake_labels)
            g_style_loss = netD.style_loss(style_fake, z_style)
            g_loss =  (g_fake_loss  + +args.l1_p *g_fake_class_loss + g_style_loss) / (2+args.l1_p)
            g_loss.backward()
            optim_G.step()
        ...
        with open(os.path.join(args.workspace_dir, 'training_loss.csv'),'w',newline='') as csvfile:
            ...
            c.writerow(['Epoch','Generator_loss','Discriminator_loss','Information_loss'])
            for i in range(len(G_L_list)):
                c.writerow([i+1,G_L_list[i],D_L_list[i],Info_list[i]])
        if (epoch) % args.model_sample == 0:
